                    ...
                    is KotlinSingleTargetExtension -> {
                        ...
                        target.compilations.forEach { compilation ->
                            if (compilation is AbstractKotlinCompilation<*>) {
                                compilation.basicTaskDataByTaskName.values.forEach { basicTaskData ->
                                    val module = IncrementalModuleEntry(
                                        project.path,
                                        compilation.moduleName,
                                        project.buildDir,
                                        basicTaskData.buildHistoryFile
                                    )
                                    dirToModule[basicTaskData.destinationDirProvider()] = module
                                    basicTaskData.javaOutputDir?.let { dirToModule[it] = module }
                        ...
                        listOf(kotlinExtension.target)
                    }

                    is KotlinMultiplatformExtension -> {
                        kotlinExtension.targets.withType(AbstractKotlinTarget::class.java).forEach { target ->
                            val mainCompilation =
                            ...
                            val jarTask = project.tasks.findByName(target.artifactsTaskName) as? AbstractArchiveTask ?: return@forEach
                            jarToModule[jarTask.archivePath] = module
                        }
                    }

                    else -> error("Unexpected Kotlin project extension.")
                }
            }
