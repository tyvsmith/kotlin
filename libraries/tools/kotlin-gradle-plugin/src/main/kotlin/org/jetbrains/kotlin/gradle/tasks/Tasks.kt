...
abstract class AbstractKotlinCompile<T : CommonCompilerArguments>() : AbstractKotlinCompileTool<T>() {
    ...
    @get:Internal
    internal val basicTaskData: AbstractKotlinCompilation<*>.BasicKotlinCompileTaskData
        get() = compilation.basicTaskDataForTask(name)

    @get:Input
    internal open var useModuleDetection: Boolean
        get() = basicTaskData.useModuleDetection
        set(value) { basicTaskData.useModuleDetection = value }

    @get:Internal
    protected val multiModuleICSettings: MultiModuleICSettings
        get() = basicTaskData.multiModuleICSettings
    ...
    override fun getDestinationDir(): File {
        return basicTaskData.destinationDirProvider()
    }

    fun setDestinationDir(provider: () -> File) {
        basicTaskData.destinationDirProvider = provider
    }

    override fun setDestinationDir(destinationDir: File) {
        basicTaskData.destinationDirProvider = { destinationDir }
    }
    ...
    internal var javaOutputDir: File?
        get() = basicTaskData.javaOutputDir
        set(value) { basicTaskData.javaOutputDir = value }
