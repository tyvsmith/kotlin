...
object KotlinMultiplatformAnalysisModeComponent {
    ...
    fun getMode(project: Project): Mode {
        ...
        val explicitRegistryOverride = Registry.get("kotlin.use.composite.analysis").key.nullize()?.toBoolean()
        if (explicitRegistryOverride == true) return Mode.COMPOSITE
        if (explicitRegistryOverride == false) return Mode.SEPARATE

        // We additionally use PropertiesComponent in order to:
        //  - allow changing value programmatically without touching Registry. It can be useful for tests, where changing registry
        //    is a bad idea in case of parallel tests)
        //  - have this setting on a per-project basis (which wouldn't have been possible if we had used one static field)
